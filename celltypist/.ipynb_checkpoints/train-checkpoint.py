...
def _prepare_data(X, labels, genes, transpose, normalize) -> tuple:
    ...
    if isinstance(X, AnnData) or (isinstance(X, str) and X.endswith('.h5ad')):
        ...
        sc.pp.normalize_total(adata) #in original CellTypist, there is also target_sum=1e4, but for consistency it was removed to normalize to median library size
        sc.pp.log1p(adata)
        indata = adata.X
        genes = adata.var_names
        labels = _to_vector(labels)
